classdef  Callback < casadi.Function
  ...
  methods
    ...
    %
      if nargin==1 && strcmp(class(varargin{1}),'SwigRef')
        ...
        if strcmp(class(self),'director_basic.Callback')
          tmp = casadiMEX(943, 0, varargin{:});
        else
          tmp = casadiMEX(943, self, varargin{:});
        end
    ...
    %
    %  Function = GET_FORWARD(self, char name, int nfwd, [char] i_names, [char] o_names, struct opts)
    %
    ...
    %
    %  Function = GET_REVERSE(self, char name, int nadj, [char] i_names, [char] o_names, struct opts)
    %
